...
public class salesServices {
    ...
    public ObservableList<Invoices> getAllInvoices() {
        ...
        try ( Connection connect = database.getConnection();  PreparedStatement ps = connect.prepareStatement(sql);  ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                invoice.setClientAdress(rs.getString("clintAdress"));
                invoice.setStatus(rs.getInt("status"));
                invoice.setRestTable(rs.getString("resturant_table"));
                invoice.setTax(rs.getDouble("tax"));
                invoice.setServices(rs.getDouble("services"));
                invoice.setDelevary(rs.getDouble("delevary"));
                invoice.setOrderNumber(rs.getInt("orderNumber"));
    ...
    public boolean deleteSaleById(int salesId, double services, double tax) {
            ...
            try (
                ...
                try ( ResultSet rs = getSalePs.executeQuery()) {
                    if (rs.next()) {
                        ...
                        double taxValue = tax * totalPrice;
                        double servicesValue = services * totalPrice;
                        totalPrice = totalPrice + taxValue + servicesValue;
                        updateInvoicePs.setDouble(1, totalPrice);
                        updateInvoicePs.setInt(2, qty);
                        updateInvoicePs.setDouble(3, taxValue);
                        updateInvoicePs.setDouble(4, servicesValue);
    ...
    public boolean returnItemById(Sales sales, String payType, int quantity, String returnReason, double tax, double services) {
        String updateInventory = "UPDATE inventory SET available = available + ? WHERE id = ?";
        ...
        try ( Connection connect = database.getConnection()) {
            ...
            try (
                ...
                if ("inventory".equalsIgnoreCase(sales.getType())) {
                    updateInventoryPs.setInt(1, sales.getQuantity());
                ...
                if (quantity == sales.getQuantity()) {
                    ...
                    // تحديث الكمية والسعر
                    updateSalePs.setInt(1, sales.getQuantity());              // خصم الكمية
                    updateSalePs.setInt(2, sales.getQuantity());              // نفس الكمية للطرح في المعادلة
                ...
                double returnBasePrice = quantity * sales.getPrice();
                double taxValue = returnBasePrice * tax;
                double servicesValue = returnBasePrice * services;
                double returnTotalPrice = returnBasePrice + taxValue + servicesValue;

                // 3- تحديث الفاتورة
                updateInvoicePs.setDouble(1, returnTotalPrice); // after discount
                updateInvoicePs.setDouble(2, returnTotalPrice); // before discount
                updateInvoicePs.setInt(3, quantity);
                updateInvoicePs.setDouble(4, taxValue);
                updateInvoicePs.setDouble(5, servicesValue);
                ...
                updateInvoicePs.setString(10, payType);
                updateInvoicePs.setDouble(11, returnTotalPrice);

                updateInvoicePs.setInt(12, sales.getInvoice_id());
                updateInvoicePs.executeUpdate();
                ...
                insertReturnPs.setInt(4, sales.getQuantity());              // العدد المرجع
