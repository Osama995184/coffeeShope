...
public class TableServices {
    ...
    public Settings getSettings() {
        String sql = "SELECT id, tax, services FROM settings LIMIT 1";
        try (Connection conn = database.getConnection(); Statement st = conn.createStatement(); ResultSet rs = st.executeQuery(sql)) {
        ...
        return null; // لو مفيش صف
    }

    public boolean updateSettings(double tax, double services) {
        String sql = "UPDATE settings SET tax = ?, services = ? WHERE id = 1";
        try (Connection conn = database.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {

            ps.setDouble(1, tax);
            ps.setDouble(2, services);
    ...
    public boolean insertDefaultSettings(double tax, double services) {
        String sql = "INSERT INTO settings (tax, services) VALUES (?, ?)";
        try (Connection conn = database.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
